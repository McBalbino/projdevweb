...
type Props = React.ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'default'|'secondary'|'destructive'|'ghost'|'outline'; size?: 'default'|'icon' }
export function Button({ className, variant='default', size='default', ...props }: Props) {
  ...
  const sizes = { default: '', icon: 'w-9 h-9 p-0' } as const
